...
function doFormat(image, toFormat, config) {
  switch (toFormat) {
    case 'jpeg':
      image.jpeg(config);
      break;
...
module.exports = function (file, config, options) {
  ...
  return new Promise((resolve, reject) => {
    image.metadata().then(metadata => {
      ...
      if (config.format instanceof Array) {

        config.format.forEach((value) => {
          var clone = image.clone();

          doFormat(clone, value, config);
        ...
        doFormat(image, toFormat, config);
