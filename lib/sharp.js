...
module.exports = async function (file, config, options) {
  ...
  try {
    ...
    try {
      ...
      throw new PluginError(PLUGIN_NAME, errPrefix + error.message)
    }
    ...
    const allOps = toFormat.map(async value => {
      try {
        const clone = convertImage(image, value, config)
    ...
    // why return only one file when converting multiple formats?
    return allOps[0]
  } catch (error) {
    throw new PluginError(PLUGIN_NAME, errPrefix + error.message)
  }
