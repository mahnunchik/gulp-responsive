      ...
      if (config.hasOwnProperty(name)) {
        if (util.isArray(config[name])) {
          config[name].forEach(function(conf) {
            preparedConfig.push(merge(true, defaultConfig, conf, {name: name}));
          });
        } else {
          preparedConfig.push(merge(true, defaultConfig, config[name], {name: name}));
        }
      }
    }
  } else {
    config.forEach(function(conf) {
      preparedConfig.push(merge(true, defaultConfig, conf));
    });
  }
